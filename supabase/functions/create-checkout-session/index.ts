...
serve(async (req) => {
  ...
  try {
    ...
    const credits = creditsMap[plan];
    console.log("💰 Credits for plan", plan, ":", credits);

    const origin = req.headers.get("origin") || "https://aiproject4you.lovable.app";

    console.log("Creating checkout session for user:", user.id, "plan:", plan, "credits:", credits);
    ...
    console.log("✅ Checkout session created:", session.id, "with", credits, "credits");
